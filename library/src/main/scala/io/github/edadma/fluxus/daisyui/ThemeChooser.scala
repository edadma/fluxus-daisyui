...
/** Global state for ThemeChooser components to remain synchronized */
object ThemeChooserState {
  // Global theme state that all instances will share
  val theme: Var[String] = Var[String](
    Option(dom.document.documentElement.getAttribute("data-theme"))
      .filter(_.nonEmpty)
      .getOrElse("light"),
  )

  // Update the theme globally
  def setTheme(newTheme: String): Unit = {
    Transaction { _ =>
      theme.set(newTheme)
    }
    dom.document.documentElement.setAttribute("data-theme", newTheme)
    dom.window.localStorage.setItem("theme", newTheme)
  }
}
...
val ThemeChooser = (props: ThemeChooserProps) => {
  // Use the global state instead of local component state
  val currentTheme = useSignal(ThemeChooserState.theme)
  ...
  useEffect(
    () => {
      ...
      if (savedIsAuto && props.respectSystemPreference) {
        ...
        ThemeChooserState.setTheme(savedTheme.get)
      } else if (props.respectSystemPreference) {
        // No saved preference, so use system preference
        applyThemeBasedOnSystemPreference()
      } else {
        // Fallback to default theme
        ThemeChooserState.setTheme(props.defaultTheme)
      }
  ...
  // Helper to set theme - use the global state manager
  def setTheme(theme: String): Unit = {
    ThemeChooserState.setTheme(theme)
  ...
  def applyThemeBasedOnSystemPreference(): Unit = {
    ...
    // Apply the theme using the global state manager
    ThemeChooserState.setTheme(preferredTheme)
    ...
    case "toggle" =>
      ...
      div(
        ...
        label(
          ...
          // Moon icon (for dark mode) - swap-on means it's shown when checkbox is checked (dark mode)
          svg(
          ...
          // Sun icon (for light mode) - swap-off means it's shown when checkbox is unchecked (light mode)
          svg(
